...
import { cn, formatCurrency, getOrderStatusColor, getTimeElapsed } from "@/lib/utils"
    ...
    >
        ...
        <div className="flex items-center gap-2">
          <div className="flex flex-col">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-primary font-medium text-sm">{tableNumber}</span>
              <span className="text-xs text-muted-foreground ml-1">#{order.orderId?.slice(-3) || "000"}</span>
            </div>
          </div>
        </div>
        <div className="flex items-center gap-1.5">
            ...
            <DropdownMenuContent align="end" className="w-40">
              {Object.values(OrderStatusEnum).map((status) => (
                ...
                >
                  ...
                  {order.status === status && <Check className="h-3 w-3 ml-auto" />}
                </DropdownMenuItem>
              ))}
            </DropdownMenuContent>
          </DropdownMenu>
      ...
      <CardContent className="p-3 pt-0 flex-1 flex flex-col">
        <div className="grid grid-cols-[auto_1fr_auto] gap-x-3 gap-y-1 text-xs">
          ...
          <div className="font-medium text-muted-foreground text-right">Price</div>
          ...
          <div className="contents col-span-3 flex-1">
            {order.orderItems.map((item) => (
              <div key={item.id} className="contents group">
                ...
                <div className="py-1 truncate group-hover:text-primary transition-colors">{item.menuItem.name}</div>
                <div className="py-1 text-right">${formatCurrency(item.menuItem.price)}</div>
              </div>
            ))}
      ...
      <CardFooter className="p-2 bg-muted/20 flex justify-between items-center shrink-0">
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  ...
                  disabled={order.status === OrderStatusEnum.CANCELLED || isStatusUpdating}
                >
        ...
        <Button
          ...
          disabled={
            order.status === OrderStatusEnum.COMPLETED || order.status === OrderStatusEnum.CANCELLED || isStatusUpdating
          }
        >
