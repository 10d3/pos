...
export function AddItemDialog({ open, onOpenChange, onAdd }: AddItemDialogProps) {
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      // Appeler la fonction onAdd si elle existe
      if (onAdd) {
        onAdd(newItem)
      }
      ...
      toast.error("Erreur", {
        description: `Une erreur est survenue lors de l'ajout du produit. ${error}`,
      })
    } finally {
      setIsLoading(false)
    }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Remplissez les informations du produit ci-dessous. Les champs marqués d&apos;un * sont obligatoires.
          </DialogDescription>
        </DialogHeader>
